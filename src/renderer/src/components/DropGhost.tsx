...
interface AddPageGhostProps extends GhostSize {
  onClick: () => void
}

export function AddPageGhost({ width, height, onClick }: AddPageGhostProps): React.JSX.Element {
  return (
    <button
      type="button"
      className="page add-ghost add-page"
      style={{ width, height }}
      title="Add page"
      onClick={(e) => {
        e.stopPropagation()
