...
export function useEditStore(): EditStore {
  ...
  // The store is the context value, so a fresh object literal here would re-render every
  // useEdits() consumer on ANY App render — busy counters, toasts, zoom, drag state, all of
  // which live in components above the provider. Memoizing on the store's own fields keeps
  // that identity stable until something in the store actually changes. Every callback below
  // is already useCallback-stable (or a useState setter), and the palettes / SHAPE_WIDTH are
  // module constants, so the dep list is exactly the mutable state this object exposes.
  return useMemo<EditStore>(
    () => ({
      overlays,
      tool,
      setTool,
