...
import {
  ...
  // Both shared helpers read the FIRST /ByteRange and /Contents in the file — here that is the
  // prior approval signature, not the appended document timestamp.
  byteRangeDigest as firstByteRangeDigest,
  cmsMessageDigest as firstCmsMessageDigest
} from './test-utils/fixtures'

const makeP12 = (passphrase: string): Uint8Array =>
  makeCredential(passphrase, { cn: 'TS Signer' }).p12

const makePdf = (): Promise<Uint8Array> => makeTestPdf('Timestamp me')
